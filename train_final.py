...
# === 0. 超参 (已被验证为最高效的配置) ===
CFG = dict(
    past_steps   = 96 * 5,  # 增加past_steps以覆盖更长的时间窗口
    future_steps = 96,
    lstm_units   = [256, 128],  # 增加LSTM单元数量以提高模型复杂度
    drop_rate    = 0.5,  # 增加Dropout率以避免过拟合
    batch_size   = 512,  # 增加batch_size以加快训练
    epochs       = 150,  # 增加epochs以充分训练
    patience     = 20,
    lr           = 5e-4  # 增加学习率
)
...
def add_time_feat(df):
    ...
    df['year'] = df['energy_date'].dt.year  # 增加年份特征
    df['quarter'] = df['energy_date'].dt.quarter  # 增加季度特征
...
def make_dataset(data, past_steps, future_steps):
    ...
    feature_cols = [
        ...
        'load_lag1', 'load_lag4', 'load_lag96', 'load_lag192',  # 增加新的lag特征
        'load_ma4', 'load_ma12', 'load_ma96', 'load_ma192',  # 增加新的ma特征
        'load_std4', 'load_std12', 'load_std96', 'load_std192',  # 增加新的std特征
        'is_holiday', 'is_month_start', 'is_month_end', 'sin_hour', 'cos_hour', 'sin_wday', 'cos_wday', 'station_enc'
    ]
